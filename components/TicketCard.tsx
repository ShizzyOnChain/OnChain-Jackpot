...
export const TicketCard: React.FC<TicketCardProps> = ({ ticket, t, previousDraws, handleClaim, claimStatus, formatDate, formatTime }) => {
    ...
    const isWinner = useMemo(() => {
        ...
        const ticketMask = ticket.numbers.reduce((mask, n) => mask | (BigInt(1) << BigInt(n)), BigInt(0));
        const winningMask = draw.winningNumbers.reduce((mask, n) => mask | (BigInt(1) << BigInt(n)), BigInt(0));
    ...
    return (
        <div className={`rounded-3xl border-2 overflow-hidden shadow-lg transition-all duration-300 hover:shadow-2xl ${isWinner && !ticket.claimed ? 'border-yellow-400 ring-4 ring-yellow-400/20' : 'dark:border-emerald-500/10 border-gray-100'}`}>
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <p className="text-[10px] opacity-60 font-mono">#{ticket.id}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 my-6">
                        {ticket.numbers.map((n: number, i: number) => <div key={i} className="h-10 w-full bg-white/20 backdrop-blur-sm rounded-lg flex items-center justify-center text-xl font-black border border-white/30 shadow-md">{n}</div>)}
                    </div>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-widest opacity-60 text-center">Draw Date</p>
                        <p className="text-center font-bold text-sm">{formatDate(ticket.drawTimestamp)} - {formatTime(ticket.drawTimestamp)}</p>
                    </div>
                </div>
            </div>
            {isWinner && (
            ...
            {isPast && !isWinner && draw && (
                <div className="p-4 text-center bg-gray-50 dark:bg-[#021411]">
                    ...
                    <div className="flex gap-1.5 justify-center flex-wrap">{draw.winningNumbers.map((n, i) => <div key={i} className="h-7 w-7 rounded-lg bg-gray-200 dark:bg-white/10 text-emerald-700 dark:text-emerald-400 flex items-center justify-center text-xs font-bold">{n}</div>)}</div>
                </div>
            )}
        </div>
    );
